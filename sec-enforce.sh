...
# */1 * * * * /usr/local/sbin/sec-enforce.sh
...
#       sec --input /var/log/nginx/access.log --conf /etc/gathera.cfg --log /var/log/gather.log --detach &

if pgrep -x "sec" >/dev/null
then
        exit 0
else
        /usr/local/bin/sec --input /var/log/nginx/access.log --conf /etc/gathera.cfg --log /var/log/gather.log --detach
fi
